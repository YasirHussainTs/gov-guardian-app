...
type RestOf<T extends IReview | NewReview> = Omit<T, 'timestamp'> & {
...
export class ReviewService {
  ...
  protected convertDateFromClient<T extends IReview | NewReview | PartialUpdateReview>(review: T): RestOf<T> {
    return {
      ...review,
      timestamp: review.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReview: RestReview): IReview {
    return {
      ...restReview,
      timestamp: restReview.timestamp ? dayjs(restReview.timestamp) : undefined,
    };
  }
