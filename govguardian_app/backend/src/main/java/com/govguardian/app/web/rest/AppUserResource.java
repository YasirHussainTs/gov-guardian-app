...
/**
 * REST controller for managing {@link com.govguardian.app.domain.AppUser}.
 */
@RestController
...
public class AppUserResource {
    ...
    public Mono<ResponseEntity<List<AppUserDTO>>> getAllAppUsers(
        ...
        return appUserService
            ...
            .zipWith(appUserService.findByCriteria(criteria, pageable).collectList())
     ...
     */
    @GetMapping("/count")
    public Mono<ResponseEntity<Long>> countAppUsers(AppUserCriteria criteria) {
        log.debug("REST request to count AppUsers by criteria: {}", criteria);
        return appUserService.countByCriteria(criteria).map(count -> ResponseEntity.status(HttpStatus.OK).body(count));
    }
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<AppUserDTO>> getAppUser(@PathVariable("id") Long id) {
